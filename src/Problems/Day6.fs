...
let part1Code (input : string) =
  let fishes = Array.create 9 0

  (input.Split(','))
  |> Array.map int
  |> Array.groupBy id
  |> Array.map (fun (k, v) -> (k, Array.length v))
  |> Array.iter (fun (k, v) -> Array.set fishes k v)

  let rec iterate (fishes : int array) iteration =
    ...
    | _ ->
      ...
      iterate nextFishes (iteration + 1)
...
let part2Code (input : string) =
  let fishes = Array.create 9 (bigint 0)

  (input.Split(','))
  |> Array.map int
  |> Array.map bigint
  |> Array.groupBy id
  |> Array.map (fun (k, v) -> (int k, Array.length v))
  |> Array.iter (fun (k, v) -> Array.set fishes k (bigint v))

  let rec iterate (fishes : bigint array) iteration =
    match iteration with
    | 256 -> Array.sum fishes
    | _ ->
      let nextFishes = [|
        fishes.[1]
        fishes.[2]
